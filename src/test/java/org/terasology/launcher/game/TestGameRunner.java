...
public class TestGameRunner {
    ...
    private void resetGameProcess () throws Exception {
        when(gameProcess.getInputStream()).thenReturn(new ByteInputStream(new byte[0], 0));
    ...
    public void testGameOutput() throws Exception {
        ...
        when(gameProcess.getInputStream()).thenReturn(new ByteInputStream(TEST_STRING.getBytes(), TEST_STRING.getBytes().length));
